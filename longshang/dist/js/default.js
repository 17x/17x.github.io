...
myOverlay.initial = function(point, schoolFullName, text) {
...
longshangApp.controller('longshangAppCtrl', ['$scope', '$rootScope', '$state', function($scope, $rootScope, $state) {
    ...
    $scope.tolianhetiTeacherPage = function(id) {
        // alert(id)
        $state.go('index.lianhetiTeacherTable', { ID: id });
    };
    $scope.tolianhetiStudentPage = function(id) {
        // alert(id)
        $state.go('index.lianhetiStudentTable', { ID: id });
    };
    ...
    /*
        测试用数据
    */
    $scope.currentLianheti = oDistrictLevel[0].cityInfo[0].districtInfo[0];
    // console.log($scope.currentLianheti)
    /*
        测试用数据结束
    */
    $scope.big = function(obj) {
        alert(998)
    }
    $scope.highLight = function(index) {
        if (document.getElementsByClassName('lianhetiPoint')[index]) {
            document.getElementsByClassName('lianhetiPoint')[index].style.backgroundPosition = '100% 0';
        }
    }

    $scope.removeHighLight = function(index) {
        if (document.getElementsByClassName('lianhetiPoint')[index]) {
            document.getElementsByClassName('lianhetiPoint')[index].style.backgroundPosition = '0 0';
        }
    }
        ...
        function statAndFilter(level) {
            for (var x = 0; x < oDistrictLevel.length; x++) {
                ...
                if (level === 1) {
                    if (myMap.loaded) {
                        console.log(998)
                        var myCompOverlay = new(myOverlay.initial(new BMap.Point(oDistrictLevel[x].coordinate[0], oDistrictLevel[x].coordinate[1]), oDistrictLevel[x].cityName));
            ...
            for (var i = 0; i < param.length; i++) {
                for (var j = 0; j < param[i].cityInfo.length; j++) {
                    ...
                    if (level === 2) {
                        // 添加当前市所有区县的地图覆盖层
                        var myCompOverlay = new(myOverlay.initial(new BMap.Point(param[i].cityInfo[j].coordinate[0], param[i].cityInfo[j].coordinate[1]), param[i].cityInfo[j].districtName));
                        myMap.addOverlay(myCompOverlay);
                    ...
                    for (var k = 0; k < param[i].cityInfo[j].districtInfo.length; k++) {
                        ...
                        if (level === 3) {
                            function ComplexCustomOverlay(point, schoolFullName, text) {
                                this._point = point;
                                this._text = text;
                                this._schoolFullName = schoolFullName;
                            }
                            ...
                            ComplexCustomOverlay.prototype.initialize = function(map) {
                                ...
                                var div = this._div = document.createElement("div");
                                div.className = 'overlayPoint lianhetiPoint';
                                var that = this;
                                div.onmouseover = function() {
                                    if (document.getElementById('schoolBriefOnMap')) {
                                        document.getElementById('schoolBriefOnMap').parentNode.removeChild(document.getElementById('schoolBriefOnMap'));
                                    }
                                    var tempDiv = document.createElement('div');
                                    tempDiv.id = tempDiv.className = 'schoolBriefOnMap';
                                    tempDiv.style.left = (this.offsetWidth - 193) + "px";
                                    tempDiv.style.bottom = this.offsetHeight + (this.offsetHeight / 2) + 4 + "px";
    ...
    return {
        ...
        link: function(scope, ele, attr, upFoo) {
            ...
            for (var i = 0; i < oDistrictLevel.length; i++) {
                var myCompOverlay = new(myOverlay.initial(new BMap.Point(oDistrictLevel[i].coordinate[0], oDistrictLevel[i].coordinate[1]), oDistrictLevel[i].cityName));
                myMap.addOverlay(myCompOverlay);
            }
            ...
            if (attr.lianheti) {
                ...
                var myCompOverlay = new(myOverlay.initial(new BMap.Point(scope.currentLianheti.coordinate[0], scope.currentLianheti.coordinate[1]), scope.currentLianheti.schoolName));
                myMap.addOverlay(myCompOverlay);
                ...
                for (var i = 0; i < scope.currentLianheti.schoolInfo.jiaoxuedian.length; i++) {
                   ...
                   var myCompOverlay = new(myOverlay.initial(new BMap.Point(curJiaoxuedian.coordinate[0], curJiaoxuedian.coordinate[1]), curJiaoxuedian.jiaoxuedianName));
                   myMap.addOverlay(myCompOverlay);
                }
            }
        }
