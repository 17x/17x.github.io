		...
		with open("cityinfo.json","wb") as f:
			...
			with open("allarea.json","wb") as f:
				...
				while i < length:
					...
					allStreetHtml = urllib.request.urlopen('http://fzwjg.com/m2/s/index.php?mod=post&action=street&cityid=' + str(id) + '&areaid=' + str(i)).read()
					isExist=os.path.exists(str(i))

					if not isExist:
						os.makedirs('street'+str(i))

					os.chdir('street'+str(i))
					level2path = os.getcwd()
					print('创建并进入二级目录'+level2path)

					with open("streetinfo.json","wb") as f:
